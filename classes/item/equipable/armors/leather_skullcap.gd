...
class_name LeatherSkullcap

func _init(crafter = null):
  #valeurs immuables (nom, poids)
  label = "Calotte de cuir"
  _name = "leather_skullcap"
  weight = 1.0
  description = "Une protection de tête rudimentaire."
  base_price = 20
  slots = ["head"]
  ...
  if crafter != null:
    bonus_def = int(crafter.get_total_attribute("leatherwork") * 0.2)
